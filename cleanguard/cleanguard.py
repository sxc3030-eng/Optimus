...
def run_scan(scan_type: str, custom_path: str = None, ws_broadcast=None):
    ...
    try:
                ...
                try:
                    ...
                    if threats:
                        ...
                        for t in threats:
                            ...
                            if HAS_TOAST:
                                toast_threat("CleanGuard Pro", t["name"], t.get("severity", "high"), fp_str)
        ...
        if HAS_TOAST:
            toast_scan("CleanGuard Pro", f"{len(SCAN.threats_found)} threats found")
...
def quarantine_file(filepath: Path, threat_name: str) -> bool:
    ...
    try:
        ...
        if HAS_TOAST:
            toast_quarantine("CleanGuard Pro", filepath.name, threat_name)
        return True
    except Exception as e:
...
try:
    from toast_notifications import toast_threat, toast_quarantine, toast_scan, toast_event
    HAS_TOAST = True
except ImportError:
    HAS_TOAST = False
...
class CleanGuardAPI:
    ...
    def _check_perm(self, action):
        if self._perm and not self._perm.check("cleanguard", action, self._auth_user):
    ...
    def quarantine_restore(self, name):
        denied = self._check_perm("quarantine")
        if denied:
            return denied
    ...
    def quarantine_delete(self, name):
        denied = self._check_perm("delete")
        if denied:
            return denied
    ...
    def perm_get_status(self):
        ...
        return {"available": True, **self._perm.get_status("cleanguard")}

    def perm_authenticate(self, password):
        if not self._perm:
            return {"success": False, "error": "permissions not available"}
        ok, user = self._perm.authenticate("cleanguard", password)
        if ok:
    ...
    def perm_enable(self, password):
        if not self._perm:
            return {"success": False, "error": "permissions not available"}
        return {"success": self._perm.enable("cleanguard", password)}

    def perm_lock(self):
        if not self._perm:
            return {"success": False}
        self._auth_user = None
        return {"success": self._perm.lock("cleanguard")}
...
╚══════════════════════════════════════════════════════╝
    ...
    def on_loaded():
        print("[✓] Dashboard chargé dans la fenêtre native")
        ...
        for fallback in range(port + 1, port + 5):
            try:
                async with websockets.serve(ws_handler, "localhost", fallback):
                    print(f"[✓] Serveur WebSocket démarré sur le port {fallback} (fallback)")
