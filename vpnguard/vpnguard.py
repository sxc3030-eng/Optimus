...
try:
    from toast_notifications import toast_connection, toast_event
    HAS_TOAST = True
except ImportError:
    HAS_TOAST = False
...
class KillSwitch:
    ...
    def enable(self, tunnel_interface: str = "") -> dict:
        ...
        try:
            ...
            if HAS_TOAST:
                toast_event("VPN Guard Pro", "Kill switch activated")
            return {"enabled": True}
        except Exception as e:
            return {"error": str(e)}
...
class VPNEngine:
    ...
    def quick_connect(self) -> dict:
        ...
        if result.get("ok"):
            ...
            if HAS_TOAST:
                toast_connection("VPN Guard Pro", "connected", CFG.wg_interface)
        return result
    ...
    def quick_disconnect(self) -> dict:
        ...
        threading.Thread(target=self._update_public_ip, daemon=True).start()
        if HAS_TOAST:
            toast_connection("VPN Guard Pro", "disconnected", CFG.wg_interface)
        return result
...
_DESTRUCTIVE_CMDS = {
    "quick_connect", "quick_disconnect", "connect_profile",
    "add_peer", "remove_peer", "import_config",
    "toggle_kill_switch", "toggle_dns_protection", "toggle_split_tunnel",
    "split_add_app", "split_remove_app",
    "delete_profile", "import_profile",
    "wg_set_config", "save_settings",
}


def _ws_perm_check(cmd: str, perm_manager) -> Optional[dict]:
    """Return an error dict if the command is blocked by permissions, else None."""
    if perm_manager is None:
        return None
    if cmd not in _DESTRUCTIVE_CMDS:
        return None
    if perm_manager.is_locked("vpnguard"):
        return {"type": "error", "message": "VPN Guard Pro is locked. Authenticate to unlock."}
    return None


async def handle_ws_command(ws, msg: dict, engine: VPNEngine, perm_manager=None):
    cmd = msg.get("cmd")

    # Permission gate for destructive operations
    blocked = _ws_perm_check(cmd, perm_manager)
    if blocked:
        await ws.send(json.dumps(blocked))
        return
...
async def ws_handler(websocket, engine):
    ...
    try:
        ...
        async for raw in websocket:
            try:
                msg = json.loads(raw)
                await handle_ws_command(websocket, msg, engine, perm_manager=PermissionManager() if HAS_PERMISSIONS else None)
...
class VPNGuardAPI:
    ...
    def send_command(self, cmd, params=None):
        ...
        if loop and loop.is_running():
            future = asyncio.run_coroutine_threadsafe(handle_ws_command(fake, msg, self._engine, perm_manager=self._perm), loop)
            try:
            ...
            try:
                asyncio.run(handle_ws_command(fake, msg, self._engine, perm_manager=self._perm))
            except Exception as e:
                return {"type": "error", "message": str(e)}
    ...
    def perm_get_status(self):
        ...
        return {
            "available": True,
            "enabled": self._perm.is_enabled("vpnguard"),
            "locked": self._perm.is_locked("vpnguard"),
            "user": self._auth_user,
        }

    def perm_authenticate(self, password):
        if not self._perm:
            return {"success": False, "error": "Permissions module not available"}
        ok = self._perm.authenticate("vpnguard", password)
        if ok:
    ...
    def perm_enable(self, password):
        if not self._perm:
            return {"success": False, "error": "Permissions module not available"}
        return {"success": self._perm.enable("vpnguard", password)}

    def perm_lock(self):
        if not self._perm:
            return {"success": False}
        self._perm.lock("vpnguard")
        self._auth_user = None
        return {"success": True}
