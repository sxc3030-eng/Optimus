...
def _fetch_city_async(ip: str):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "Optimus/1.9"})
...
def load_et_rules_online(ruleset_key: str) -> dict:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "Optimus/1.6"})
...
def vuln_cve_lookup(cve_id: str) -> dict:
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "Optimus/4.0"})
...
def block_ip_os(ip: str, reason: str):
    ...
    # Toast notification for block
    if HAS_TOAST:
        toast_block("NetGuard Pro", ip, reason)
...
def add_threat(src_ip: str, threat_type: str, description: str, severity: str, rule_key: str = None):
    ...
    # Toast notification for threat
    if HAS_TOAST:
        toast_threat("NetGuard Pro", threat_type, severity, description)
...
def scan_lan() -> list:
    ...
    try:
        ...
        if HAS_TOAST:
            toast_scan("NetGuard Pro", f"{len(devices)} appareils trouvés sur le réseau")
...
async def handle_ws_command(ws, msg: dict):
    ...
    _destructive_cmds = {"block_ip", "unblock_ip", "clear_threats", "toggle_rule",
                         "update_param", "scan_lan", "record_start", "record_stop"}
    if cmd in _destructive_cmds and HAS_PERMISSIONS:
        _pm = PermissionManager()
        if not _pm.check("netguard", "execute", None):
            await ws.send(json.dumps({"type": "error", "error": "Permission denied / Accès refusé"}))
            return
...
class NetGuardAPI:
    ...
    def perm_authenticate(self, username, password):
        if not self._perm:
    ...
    def perm_enable(self, username, password):
        if not self._perm:
