...
try:
    from toast_notifications import toast_integrity, toast_event
    HAS_TOAST = True
except ImportError:
    HAS_TOAST = False
...
class FIMEngine:
    ...
    def _check_perm(self, perm_type="execute"):
        ...
        if self._perm.locked and not self._auth_user:
            return False
        return self._perm.check(self._auth_user, "fim", perm_type)
        ...
        while self._running:
            if time.time() - self._last_scan > self._scan_interval:
                ...
                if alerts:
                    ...
                    if HAS_TOAST:
                        for alert in alerts:
                            toast_integrity("FIM", alert.get("filename", ""), alert.get("type", "modified"), alert.get("message", ""))
            ...
            try:
                ...
                if cmd == "get_state":
                    ...
                    if not _engine._check_perm("execute"):
                        await websocket.send(json.dumps({"type": "error", "error": "Permission denied: 'execute' on fim"}))
                    else:
                        count = _engine.rebuild_baseline()
                        await websocket.send(json.dumps({"type": "baseline_rebuilt", "count": count}))
                elif cmd == "perm_get_status":
                    if _engine._perm:
                        await websocket.send(json.dumps({"type": "perm_status", "enabled": _engine._perm.is_enabled(), "locked": getattr(_engine._perm, "locked", False), "auth_user": _engine._auth_user}))
                    else:
                        await websocket.send(json.dumps({"type": "perm_status", "enabled": False, "locked": False, "auth_user": None}))
                elif cmd == "perm_enable":
                    if _engine._perm:
                        _engine._perm.enable()
                        await websocket.send(json.dumps({"type": "perm_enabled", "ok": True}))
                    else:
                        await websocket.send(json.dumps({"type": "error", "error": "Permissions module not available"}))
                elif cmd == "perm_authenticate":
                    user = msg.get("user", "")
                    password = msg.get("password", "")
                    if _engine._perm and _engine._perm.authenticate(user, password):
                        _engine._auth_user = user
                        await websocket.send(json.dumps({"type": "perm_authenticated", "ok": True, "user": user}))
                    else:
                        await websocket.send(json.dumps({"type": "perm_authenticated", "ok": False, "error": "Authentication failed"}))
                elif cmd == "perm_lock":
                    if _engine._perm:
                        _engine._perm.lock()
                        _engine._auth_user = None
                        await websocket.send(json.dumps({"type": "perm_locked", "ok": True}))
                    else:
                        await websocket.send(json.dumps({"type": "error", "error": "Permissions module not available"}))
                    ...
                    password = msg.get("password", "")
                    if _engine._perm and _engine._perm.authenticate(user, password):
                        _engine._perm.unlock()
                        _engine._auth_user = user
                        await websocket.send(json.dumps({"type": "perm_unlocked", "ok": True}))
                    else:
                        await websocket.send(json.dumps({"type": "perm_unlocked", "ok": False, "error": "Authentication failed"}))
                elif cmd == "get_alerts":
