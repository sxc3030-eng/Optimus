...
AGENTS = {
    ...
    "redteam": {
        "name": "RedTeam Agent",
        "port": 8870,
        "script": os.path.join(BASE_DIR, "redteam", "redteam.py"),
        "icon": "crosshairs",
        "color": "#ff2222",
        "description": "Tests de penetration & audit offensif",
    },
    "siem": {
        "name": "SIEM Agent",
        "port": 8880,
        "script": os.path.join(BASE_DIR, "siem", "siem.py"),
        "icon": "brain",
        "color": "#ff8800",
        "description": "Collecte & correlation de logs securite",
    },
    "sandbox": {
        "name": "Sandbox Agent",
        "port": 8890,
        "script": os.path.join(BASE_DIR, "sandbox", "sandbox.py"),
        "icon": "box",
        "color": "#aa44ff",
        "description": "Analyse dynamique de fichiers suspects",
    },
    "mobile_gateway": {
        "name": "Mobile Gateway",
        "port": 8895,
        "script": os.path.join(BASE_DIR, "mobile_gateway", "gateway.py"),
        "icon": "mobile-alt",
        "color": "#00ccff",
        "description": "Passerelle mobile & monitoring distant",
    },
}
...
class ThreatCorrelator:
    ...
    def _add_alert(self, category: str, event):
        ...
        # Toast notification for correlated threats
        if HAS_TOAST:
            description = str(event.data.get("description", event.channel))
            toast_threat("SentinelOS Cortex", "Threat Correlated", event.severity or "high", description)
...
class CortexAPI:
    ...
    def start_agent(self, agent_key: str) -> str:
        if self._perm and self._perm.is_enabled() and self._perm.locked:
            if not self._perm.check(self._auth_user, "cortex", "execute"):
                return json.dumps({"error": "Permission denied — unlock required"})
        ...
        if HAS_TOAST:
            toast_event("SentinelOS Cortex", f"Agent {agent_name} started")
    ...
    def stop_agent(self, agent_key: str) -> str:
        ...
        agent_name = AGENTS.get(agent_key, {}).get("name", agent_key)
        if HAS_TOAST:
            toast_event("SentinelOS Cortex", f"Agent {agent_name} stopped")
        return json.dumps({"success": ok})
    ...
    def restart_agent(self, agent_key: str) -> str:
        ...
        agent_name = AGENTS.get(agent_key, {}).get("name", agent_key)
        if HAS_TOAST:
            toast_event("SentinelOS Cortex", f"Agent {agent_name} restarted")
        return json.dumps({"success": ok})
    ...
    def perm_get_status(self):
        ...
        return json.dumps({"enabled": self._perm.is_enabled(), "locked": self._perm.locked, "available": True, "user": self._auth_user})
...
async def handle_ws_command(ws, msg: dict):
    ...
    # ─── Permission commands ─────────────────────────────────────
        ...
        result = _api.perm_get_status()
        await ws.send(result)

    elif cmd == "perm_enable":
        result = _api.perm_enable(msg.get("user", ""), msg.get("pw", ""))
        await ws.send(result)

    elif cmd == "perm_authenticate":
        result = _api.perm_authenticate(msg.get("user", ""), msg.get("pw", ""))
        await ws.send(result)

    elif cmd == "perm_lock":
        result = _api.perm_lock()
        await ws.send(result)

    elif cmd == "perm_unlock":
        result = _api.perm_unlock(msg.get("pw", ""))
        await ws.send(result)
    ...
    if not os.path.exists(dashboard_path):
        ...
        try:
            ...
            window = webview.create_window(
                f"Optimus Command Center — Cybersecurity Suite",
                dashboard_path,
                js_api=_api,
                width=1400,
                height=900,
