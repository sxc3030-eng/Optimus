...
try:
    from toast_notifications import toast_threat, toast_event, toast_scan
    HAS_TOAST = True
except ImportError:
    HAS_TOAST = False
...
class MailShieldEngine:
    ...
    def send_email(self, to, subject, body_html, cc="", bcc="", attachments=None, account=None):
        # Permission check for send
        if self._perm and not self._perm.check("mailshield", "send_email", self._auth_user):
            return False, "Permission denied: send_email requires authentication"
    ...
    def delete_email(self, email_id, permanent=False):
        ...
        if self._perm and not self._perm.check("mailshield", "delete_email", self._auth_user):
            return False, "Permission denied: delete_email requires authentication"
...
<div>{orig['body_html'] or '<pre>' + (orig['body_text'] or '') + '</pre>'}</div>"""
            ...
            # Permission check for modify settings
            if engine._perm and not engine._perm.check("mailshield", "modify_settings", engine._auth_user):
                self.json_response({"success": False, "message": "Permission denied: modify_settings requires authentication"})
                return
            ...
            if result.get("is_phishing") and HAS_TOAST:
                details = f"Score: {result.get('score', 0)}, From: {body.get('from_email', 'unknown')}"
                toast_threat("MailShield Pro", "Phishing", "high", details)
            ...
            if row:
                ...
                if HAS_TOAST:
                    toast_event("MailShield Pro", "Spam blocked: " + row["subject"])
            conn.close()
            self.json_response({"success": True})
            ...
            if HAS_STARTUP_UTILS:
                ...
                self.json_response({"enabled": False, "available": False})
        # ── Permission API ─────────────────────────────────────────────────
        elif path == "/api/perm/status":
            if engine._perm:
                self.json_response(engine._perm.get_status("mailshield"))
            else:
                self.json_response({"enabled": False, "locked": False, "available": False})
        elif path == "/api/perm/authenticate":
            if engine._perm:
                user = body.get("user", "")
                pwd = body.get("password", "")
                ok = engine._perm.authenticate("mailshield", user, pwd)
                if ok:
                    engine._auth_user = user
                self.json_response({"success": ok})
            else:
                self.json_response({"success": False, "message": "Permissions module not available"})
        elif path == "/api/perm/enable":
            if engine._perm:
                ok = engine._perm.enable("mailshield", body.get("user", ""), body.get("password", ""))
                self.json_response({"success": ok})
            else:
                self.json_response({"success": False, "message": "Permissions module not available"})
        elif path == "/api/perm/lock":
            if engine._perm:
                engine._perm.lock("mailshield")
                engine._auth_user = None
                self.json_response({"success": True})
            else:
                self.json_response({"success": False, "message": "Permissions module not available"})
        elif path == "/api/perm/unlock":
            if engine._perm:
                user = body.get("user", "")
                pwd = body.get("password", "")
                ok = engine._perm.unlock("mailshield", user, pwd)
                if ok:
                    engine._auth_user = user
                self.json_response({"success": ok})
            else:
...
def main_webview():
    ...
    window = webview.create_window(
        f"MailShield Pro — Optimus Suite",
